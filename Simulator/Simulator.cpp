...
bool Simulator::readBoard(std::string& input_file, BoardInfo& board_info) {
    ...
    board_info.map_name = extractBaseName(input_file);
    file.close();
    return true;
}
...
// ================================== Competitive =========================================
 void Simulator::runCompetitive(){
    std::vector<BoardInfo> maps = readMapsFolder(config.gameMapsFolder);
    int maps_size = maps.size();
    int algorithms_size = AlgorithmRegistrar::getAlgorithmRegistrar().count();
    if (algorithms_size < 2) {
    ...
    if (num_threads > maps_size * algorithms_size) {
        num_threads = maps_size * algorithms_size; // So that there wouldn't be unecessary amount of threads
    }
    if (num_threads == 1) {
        runCompetitiveThread(0, num_threads, maps_size, algorithms_size, std::ref(results), std::ref(maps));
    }
    else{
        std::vector<std::thread> threads;
        for (int i = 0; i < num_threads; ++i) {
            threads.emplace_back([this, i, num_threads, maps_size, algorithms_size, &results, &maps]() {
                this->runCompetitiveThread(i, num_threads, maps_size, algorithms_size, results, maps);
                    });
        }
        for (auto& thread : threads) {
            thread.join();
    ...
    std::vector<std::pair<std::string, int>> scores = getAlgorithmScores(results, algorithms_size, maps_size);
    writeCompetitiveOutput(scores);
 }

 void Simulator::runCompetitiveThread(int thread_id, int num_threads, int maps_size, int algorithms_size, std::vector<GameResult>& results, std::vector<BoardInfo>& maps){
    auto& game_manager_registrar = GameManagerRegistrar::getGameManagerRegistrar();
    auto& algorithm_registrar = AlgorithmRegistrar::getAlgorithmRegistrar();
    // Default to first game manager
    auto game_manager_registrar_entry = game_manager_registrar.at(0);
    for (int i = thread_id; i < maps_size * algorithms_size; i += num_threads) {
        int map_index = i / algorithms_size;
        int algorithm1_index = i % algorithms_size;
        int algorithm2_index = (algorithm1_index + 1 + map_index % (algorithms_size - 1)) % algorithms_size;
        ...
        std::unique_ptr<AbstractGameManager> game_manager = game_manager_registrar_entry.createGameManager(config.verbose);
        BoardInfo& board_info = maps.at(map_index);
        auto algorithm1_registrar_entry = algorithm_registrar.at(algorithm1_index);
        auto algorithm2_registrar_entry = algorithm_registrar.at(algorithm2_index);
        std::unique_ptr<Player> player1 = algorithm1_registrar_entry.createPlayer(1,board_info.map_width, board_info.map_height, board_info.max_steps, board_info.num_shells);
        std::unique_ptr<Player> player2 = algorithm2_registrar_entry.createPlayer(2,board_info.map_width, board_info.map_height, board_info.max_steps, board_info.num_shells);
        TankAlgorithmFactory player1_tank_algo_factory = algorithm1_registrar_entry.getTankAlgorithmFactory();
        TankAlgorithmFactory player2_tank_algo_factory = algorithm2_registrar_entry.getTankAlgorithmFactory();
        std::string map_name = board_info.map_name;
        std::string name1 = extractBaseName(algorithm1_registrar_entry.name());
        std::string name2 = extractBaseName(algorithm2_registrar_entry.name());
        GameResult result = game_manager->run(board_info.map_width, board_info.map_height, board_info.map, map_name ,board_info.max_steps, board_info.num_shells, *player1.get(), name1 ,*player2.get(), name2 , player1_tank_algo_factory, player2_tank_algo_factory);
        results[i] = std::move(result);
    }
}

 std::vector<BoardInfo> Simulator::readMapsFolder(const std::string& folderPath) {
    std::vector<BoardInfo> maps;
    for (const auto& entry : std::filesystem::directory_iterator(folderPath)) {
        if (entry.is_regular_file() && entry.path().extension() == ".txt") {
            BoardInfo board_info;
            std::string filename = entry.path().string();
            if (!readBoard(filename, board_info)) {
                std::cout << "Usage: Map file couldn't be read successfully" << std::endl; // erroer message TODO - FIX
            }
            maps.push_back(board_info);
        }
    }
    return maps;
}

std::vector<std::pair<std::string, int>> Simulator::getAlgorithmScores(std::vector<GameResult>& results, int algorithms_size, int maps_size) {
    auto& algorithm_registrar = AlgorithmRegistrar::getAlgorithmRegistrar();
    std::vector<std::pair<std::string, int>> scores(algorithm_registrar.count());
    for (int algorithm = 0; algorithm < algorithms_size; algorithm++) {
        auto algorithm_registrar_entry = algorithm_registrar.at(algorithm);
        scores.at(algorithm).first = extractBaseName(algorithm_registrar_entry.name());
    }
    for (int map_index = 0; map_index < maps_size; map_index++) {
        for (int algorithm1 = 0; algorithm1 < algorithms_size; algorithm1++) {
            int algorithm2 = (algorithm1 + 1 + map_index % (algorithms_size - 1)) % algorithms_size;
            int result_index = map_index * algorithms_size + algorithm1;
            int winner = results[result_index].winner;
            if (winner == -1) continue;
            else {
                if (winner == 1) scores.at(algorithm1).second += 3;
                else if (winner == 2) scores.at(algorithm2).second += 3;
                else if (winner == 0) ++scores.at(algorithm1).second , ++scores.at(algorithm2).second;
            }
        }
    }
...
void Simulator::writeCompetitiveOutput(std::vector<std::pair<std::string, int>> scores){
    std::filesystem::path folder = config.algorithmsFolder;
    std::string filename = "competition_" + UserCommon_322996059_211779582::generateTimeBasedString() + ".txt";
    std::filesystem::path full_path = folder / filename;
    std::ofstream file(full_path);

    if (!file) {
        std::cout << "The competition result file cannot be created in the folder: " << config.algorithmsFolder << std::endl;
        printCompetitiveOutput(scores);
    }
    else{
        file << "game_maps_folder=" << extractBaseName(config.gameMapsFolder) << std::endl;
        file << "game_manager=" << extractBaseName(config.gameManagerFile) << std::endl;
        file << std::endl;
...
void Simulator::printCompetitiveOutput(std::vector<std::pair<std::string, int>> scores){
    std::cout << "game_maps_folder=" << extractBaseName(config.gameMapsFolder) << std::endl;
    std::cout << "game_manager=" << extractBaseName(config.gameManagerFile) << std::endl;
    std::cout << std::endl;
    for (std::pair<std::string, int> score : scores) {
